...
import { deleteWithAuthenticate } from "@/lib/networks/AxiosWrapper"
...
import { getUserName } from "@/lib/utils/CookieWrapper"
...
import { CDN_URL, SITE_URL } from "@/config/Constants"
...
export const PostItem: React.FC<any> = (props) => {
  const history = useRouter()
  const name = history.query.name
  const id = history.query.id as unknown as number
  ...
  const genAvatar = () => {
    return avatarSrc !== "" ? (
      <Link href={"/users/" + postItem.user_name} style={hideLinkUnderLine}>
    ...
    <Container disableGutters fixed sx={{ width: "100hv" }}>
      <Toolbar sx={{ borderBottom: 1, width: "100hv", borderColor: "divider" }}>
        <CardHeader
          avatar={genAvatar()}
          title={
            <Link href={"/users/" + postItem.user_name}>
              {"@" + postItem.user_name}
            </Link>
          }
          sx={{ height: "100%" }}
